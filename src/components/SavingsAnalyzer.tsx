...
import { TrendingDown, TrendingUp, AlertTriangle, Check, RotateCcw, Loader2, BarChart3, Bug } from "lucide-react";
    ...
    return (
      <div className="glass-card rounded-3xl p-8 text-center">
        ...
        <p className="text-muted-foreground">
          Log in to track coverage. Browse titles to see which streaming services cover the most content you care about.
        </p>
      </div>
    );
  }
      ...
      {debugMode && (
        <div className="mb-6 p-4 rounded-xl bg-yellow-500/10 border border-yellow-500/30 text-sm font-mono space-y-1">
          <div className="flex items-center gap-2 mb-2 text-yellow-400 font-semibold">
            <Bug className="w-4 h-4" /> Debug Info
          </div>
          ...
          <p className="text-muted-foreground">last trackTitle: <span className="text-foreground">
            {lastResult ? JSON.stringify(lastResult) : "none"}
          </span></p>
        </div>
      )}
      ...
      {stats.totalTitles === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-muted-foreground/70 text-sm mt-2">
            Search for movies and TV shows, then click into their details to build your coverage data.
          </p>
        </div>
      ) : (
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                      {provider.count} of {stats.totalTitles} explored titles available
                    </p>
                  </div>
          ...
          {sortedProviders.length > 0 && (
            <div className="mt-6 p-6 rounded-2xl bg-gradient-to-r from-accent/10 to-primary/10 border border-accent/30">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="font-display font-semibold text-foreground">Top pick: {sortedProviders[0].name}</p>
                  <p className="text-sm text-muted-foreground">Covers {sortedProviders[0].percent}% of the titles you've explored</p>
                </div>
              </div>
            </div>
          )}
